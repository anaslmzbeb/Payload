...
func (this *Api) ReadLine(echo bool) (string, error) {
	buf := make([]byte, 1024)
	bufPos := 0

	
	if !echo {
		this.conn.Write([]byte("\xFF\xFB\x01")) // IAC WILL ECHO (disable echo)
	} else {
		this.conn.Write([]byte("\xFF\xFC\x01")) // IAC WONT ECHO (enable echo)
	}
